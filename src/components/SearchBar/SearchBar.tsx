...
export const SearchBar = ({ scrolled }: SearchBarProps) => {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (searchBar.length >= 1) {
        console.log(
          `I can see you're not typing. I can use the Search bar: "${searchBar}" now!`
        );
        const rexUID = localStorage.getItem("rexUID");

        fetch(URL + "/api/get-users?uid=" + rexUID + "&text=" + searchBar)
          .then((res) => res.json())
          .then((json) => {
            ...
            <div id="results">
              <Scrollbars autoHide>
                {searchResults
                  .filter((u) => u.is_user)
                  .map((u) => (
                    <div id="row" onClick={() => OpenUserPage(u.id)}>
                      <img src={u.profile_image} alt="pro-img" id="pro-img" />
                      {u.is_user ? (
                        <div id="text">
                        ...
                        <div id="text">{u.phone_number}</div>
                      )}
                    </div>
                  ))}
              </Scrollbars>
